...
def build_vae(intermediate_dim=512, latent_dim=2):
    ...
    # encoder first
    inputs = layer.Input(shape=(image_size), name='encoder_input')
    x = layer.Dense(intermediate_dim, activation='relu')(inputs)

    # latent mean and variance
    z_mean = layer.Dense(latent_dim, name='z_mean')(x)
    z_log_var = layer.Dense(latent_dim, name='z_log_var')(x)
    ...
    z = layer.Lambda(sampling, output_shape=(latent_dim,), name='z')([z_mean, z_log_var])
    ...
    # decoder
    latent_inputs = layer.Input(shape=(latent_dim), name='z_sampling')
    x = layer.Dense(intermediate_dim, activation='relu')(latent_inputs)
    outputs = layer.Dense(image_size, activation='sigmoid')(x)
...
def sampling(args: tuple):
    ...
    :return tensor: sampled latent vector z
    """
...
# implement the plot latent distribution
def plot_latent_distribution(encoder, x_test, y_test, batch_size=128):
    ...
    for i in np.unique(y_test):
        plt.scatter(z_mean[y_test == i, 0], z_mean[y_test == i, 1], marker=MarkerStyle(markers[i], fillstyle='none'))
...
def plot_generated_images(decoder):
    ...
    for i, yi in enumerate(grid_y):
        for j, xi in enumerate(grid_x):
            ...
            x_decode = decoder.predict(z_sample)
    ...
    plt.xticks(ticks=pixel_range, labels=sample_range_x)
    plt.yticks(ticks=pixel_range, labels=sample_range_y)
    plt.xlabel('z[0]')
    plt.ylabel('z[1]')
